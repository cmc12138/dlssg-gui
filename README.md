...
| 逐字节进度 | 下载/解压/写入都有实时进度：界面顶栏常驻一条进度条，显示当前文件字节数（如 `12.3 MB / 28.6 MB`）、第几项、总体百分比；下载按钮上也会跟着跳百分比，不会出现"卡在下载中不知道有没有在动" |
| 数据目录清理 | 一键列出可清理项：同来源的旧版本运行库、导入中断留下的孤立目录、当前注入记录已不引用的旧备份、下载临时目录。**更新/重新导入时自动删掉同来源的旧版本**（有游戏还在用就保留）。只动 `%APPDATA%\dlssg-gui`，**游戏目录一律不碰** |
...
- `npm run typecheck` 通过；`npm test` **30 项全通过**（ini 往返与档位/默认值、三态键、文件夹与 ZIP 导入、
  注入→状态→改配置→拒绝还原→强制还原全流程、多代理不再阻塞、重复条目判定与合并（含备份迁移后仍能还原）、
  UE/Unity 两种游戏目录布局的探测、REFramework 识别与安装/卸载（含 `dinput8.dll` 备份还原）、
  下载逐字节进度、数据目录清理（孤立目录 / 被取代的旧包 / 有游戏在用的包必须保留）、git blob 哈希、上游树解析）。
